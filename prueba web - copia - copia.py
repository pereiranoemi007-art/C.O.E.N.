...
def reporte_presupuesto(evento_id):
    ...
    # calcular presupuesto por momento usando precio de Articulos (no precios de compra)
    rows = db.fetchall('''
    ...
    # detalle por momento y articulo (opcional)
    details = db.fetchall('''
        SELECT COALESCE(ed.momento, 'Sin momento') AS momento, a.nombre, SUM(cd.cant_jov+cd.cant_dor+cd.cant_var) AS cantidad, a.precio, (SUM(cd.cant_jov+cd.cant_dor+cd.cant_var) * COALESCE(a.precio,0)) AS subtotal
        FROM EventoDetalle ed
        JOIN ComidaDetalle cd ON ed.comida_id = cd.comida_id
        JOIN Articulos a ON cd.articulo_id = a.id
        WHERE ed.evento_id = ?
    ...
    table = Table(data)
    table.setStyle([('GRID', (0,0), (-1,-1), 1, colors.black), ('BACKGROUND', (0,-1), (-1,-1), colors.lightgreen)])
    elements.append(table)
    ...
    for d in details:
        ddata.append([d['momento'], d['nombre'], d['cantidad'], f"{(d['precio'] or 0):.2f}", f"{(d['subtotal'] or 0):.2f}"])
    ...
    buf.seek(0)
    safe = re.sub(r'[^a-zA-Z0-9_ ]', '', nombre).replace(' ', '_')
    return send_file(buf, mimetype='application/pdf', as_attachment=True, download_name=f'Presupuesto_{safe}.pdf')
